...
export default function App() {
  ...
  const handleGuardar = () => {
    ...
    if (editId) {
      guardarProductos(
        productos.map((p) => (p.id === editId ? { ...p, nombre, precio } : p))
      );
      setEditId(null);
    } else {
      guardarProductos([
  ...
  const handleEditar = (item: { id: string; nombre: string; precio: string }) => {
  ...
  const confirmarEliminacion = (id: string) => {
    Alert.alert('Confirmar', '¿Deseas eliminar este producto?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Eliminar',
        style: 'destructive',
        onPress: () => guardarProductos(productos.filter((p) => p.id !== id)),
      },
    ]);
  };
